...
type TodoRepository interface {
	...
	Save(ctx context.Context, todo *todomodel.Todo) error

	// FindByID は、指定されたIDを持つTodoエンティティを検索します。
	FindByID(ctx context.Context, id todomodel.TodoID) (*todomodel.Todo, error)

	// Delete は、指定されたIDを持つTodoエンティティを削除します。
	Delete(ctx context.Context, id todomodel.TodoID) error
